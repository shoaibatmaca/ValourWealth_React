...
function MarketDetails() {
  const [news, setNews] = useState([]);
  const API_BASE_URL = process.env.REACT_APP_API_URL.replace(/\/$/, ""); // Ensure no trailing slash
  ...
  useEffect(() => {
    const fetchNews = () => {
      axios
        .get(`${API_BASE_URL}/market-news/?query=US%20Stock%20Market&num_results=5`)
        .then((response) => {
          const newsResults = response.data?.results || []; // Ensure it's an array
          console.log("API Response:", newsResults); // Debugging
          setNews(newsResults.slice(0, 5)); // Slice only if array exists
        })
        .catch((error) => {
          console.error("Error fetching news:", error);
